...
tm Clock::getTm(std::chrono::system_clock::time_point const& time) {
    tm converted{};
...
tm Clock::getTm(time_t const& time) {
    return *localtime(&time);
}

time_t Clock::getTimet(std::chrono::system_clock::time_point const& time) {
    time_t converted = std::chrono::system_clock::to_time_t(time);
    return converted;
}

std::chrono::system_clock::time_point Clock::convertToTimePoint(time_t const& time) {
